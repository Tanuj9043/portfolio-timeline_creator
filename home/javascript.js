...
$(document).ready(function(){
	...
	$.ajax({url: '/home/getdata', success: function (data) {
		DATA = data;
		$('#topname').html(DATA.name);
		$('#quotename').html('- '+DATA.name);
		$('#fbaboutme').val(DATA.about);
	...
	function addtimelinedata() {
		...
		if(data.education==undefined){
			...
			for(let i=0; i<education.length; i++){
				data += `<li class="container-fluid px-0 mt-3 mb-3 d-flex" id="li${cnt}">
							<div class="container-fluid px-0 mx-2" style="width: 10%; margin: auto;">
								<div class="container-fluid mb-1 ubtn" id="ubtn${cnt}"></div>
								<div class="container-fluid mt-1 dbtn" id="dbtn${cnt}"></div>
							</div>
		...
		let y = cnt-1;
		$('#li'+y).css({'display' : 'none'});
		let x=education.length - 1;
		for(let i=0; i<education.length; i++){
			$('#tlhead'+i).val(education[x].school.name+', '+education[x].type);
			x--;
		}
		setbuttons();
	}
